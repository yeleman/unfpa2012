...
def serialize_childrenmortalityreport(report):
    ...
    report_data.update({'reporting_location':
                        ...
                        'death_location':
                        get_entity_for(report_data.get('death_location'))})

    # dates
    report_data.update({'dob':
                        datetime_to(report_data.get('dob')),
                        'dod':
                        datetime_to(report_data.get('dod'))})
...
class Command(BaseCommand):
    help = ugettext_lazy("Export pnlp2011 ChildrenMortalityReport to JSON")

    def handle(self, *args, **kwargs):

        data = []
        fileo = open('unfpa_children_death_reports.json', 'w')

        for report in ChildrenMortalityReport.objects.all():
            report_data = serialize_childrenmortalityreport(report)
        ...
        print("ChildrenMortalityReport complete")
